    ...
    try {
        ...
        if (error instanceof jwt.TokenExpiredError) {
            try {
                ...
                const decoded = jwt.verify(accessToken, process.env.ACCESS_TOKEN_SECRET || "access_secret");
                const user = await prisma.user.findUnique({
                    where: { id: decoded.id },
                    select: {
                        id: true,
                ...
                return next(new ApiError(401, refreshError?.message || "Unauthorized request"));
            }
        }
