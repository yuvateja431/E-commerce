...
class CouponService {
    static async createCoupon(data, createdBy) {
        ...
        if (data.discountType === 'PERCENTAGE' && data.discountValue > 100) {
            throw new ApiError_1.ApiError(400, "Percentage discount cannot exceed 100%");
        }
        if (new Date(data.expiryDate) <= new Date(data.validFrom || new Date())) {
            throw new ApiError_1.ApiError(400, "Expiry date must be after valid from date");
        }
    ...
    static async validateCoupon(code, cartTotal, userId) {
        ...
        if (coupon.status !== 'ACTIVE') {
            throw new ApiError_1.ApiError(400, `Coupon is ${coupon.status.toLowerCase()}`);
        }
        const now = new Date();
        if (now < new Date(coupon.validFrom)) {
            throw new ApiError_1.ApiError(400, "Coupon is not yet valid");
        }
        ...
        if (coupon.minOrderAmount && cartTotal < coupon.minOrderAmount) {
            throw new ApiError_1.ApiError(400, `Minimum order amount of ₹${coupon.minOrderAmount} required`);
        }
        if (userId && coupon.applicableUsers.length > 0 && !coupon.applicableUsers.includes(userId)) {
            throw new ApiError_1.ApiError(400, "You are not eligible to use this coupon");
        }
        if (userId && coupon.perUserLimit) {
            ...
            if (usageCount >= coupon.perUserLimit) {
                throw new ApiError_1.ApiError(400, `You have reached the maximum usage limit for this coupon`);
            }
        }
        // Calculate discount
