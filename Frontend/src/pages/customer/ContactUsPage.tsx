...
import type { ContactSettings, CreateContactMessageRequest } from "../../types/cms";
import {
  FiMail,
  FiPhone,
  FiMapPin,
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 bg-gradient-to-br from-slate-900 to-indigo-950 text-white rounded-3xl p-8 sm:p-10 shadow-xl space-y-8 relative overflow-hidden">
            ...
            {loadingSettings ? (
              <div className="flex items-center space-x-3 text-indigo-200 py-6">
              ...
              <div className="pt-4 border-t border-white/10 space-y-3">
                ...
                <div className="flex items-center space-x-3">
                  {settings.facebookUrl && (
                    <a href={settings.facebookUrl} target="_blank" rel="noreferrer" className="p-2.5 bg-white/10 hover:bg-white/20 rounded-xl transition text-white">
                      <FaFacebook className="w-4 h-4" />
                    </a>
                  )}
                  {settings.instagramUrl && (
                    <a href={settings.instagramUrl} target="_blank" rel="noreferrer" className="p-2.5 bg-white/10 hover:bg-white/20 rounded-xl transition text-white">
                      <FaInstagram className="w-4 h-4" />
                    </a>
                  )}
                  {settings.twitterUrl && (
                    <a href={settings.twitterUrl} target="_blank" rel="noreferrer" className="p-2.5 bg-white/10 hover:bg-white/20 rounded-xl transition text-white">
                      <FaTwitter className="w-4 h-4" />
                    </a>
                  )}
                  {settings.youtubeUrl && (
                    <a href={settings.youtubeUrl} target="_blank" rel="noreferrer" className="p-2.5 bg-white/10 hover:bg-white/20 rounded-xl transition text-white">
                      <FaYoutube className="w-4 h-4" />
                    </a>
                  )}
                  {settings.googleMapsUrl && (
                    <a href={settings.googleMapsUrl} target="_blank" rel="noreferrer" className="p-2.5 bg-white/10 hover:bg-white/20 rounded-xl transition text-white" title="Google Maps">
            ...
            {submittedSuccess ? (
              <div className="py-12 text-center space-y-4">
                ...
                <button
                  ...
                  className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition shadow-xs"
                >
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full px-4 py-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition text-slate-900 placeholder-slate-400"
                    />
                  </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    required
                    className="w-full px-4 py-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition text-slate-900 placeholder-slate-400 resize-y"
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full sm:w-auto px-8 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-extrabold rounded-xl transition flex items-center justify-center space-x-2 disabled:opacity-50 shadow-md shadow-indigo-500/20"
                >
                  {submitting ? (
                    <>
