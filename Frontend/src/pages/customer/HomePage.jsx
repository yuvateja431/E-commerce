...
import { FiArrowRight, FiTruck, FiShield, FiHeadphones, FiStar, FiRefreshCw, FiPackage, FiZap, FiHeart, FiShoppingCart } from "react-icons/fi";
...
export const HomePage = () => {
    ...
    return (<div className="overflow-x-hidden bg-[#FAFAFA]">

      {/* ═══════════════════════════════════════════
            LUXURY 3D HERO SECTION (DARK MODE SHOWCASE)
        ═══════════════════════════════════════════ */}
      <section onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className="relative overflow-hidden min-h-[700px] flex items-center pt-10 pb-24 bg-[#050505] text-white select-none border-b border-slate-800/40">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex flex-col lg:flex-row items-center gap-8 lg:gap-12 min-h-[640px] py-8 lg:py-12">
          ...
          <div className="flex-1 z-20 max-w-xl text-center lg:text-left">
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gradient-to-r from-amber-500/10 to-amber-400/5 border border-amber-400/30 text-amber-300 text-[11px] font-extrabold tracking-[0.2em] uppercase mb-8 backdrop-blur-md shadow-[0_0_15px_rgba(212,175,55,0.15)]">
              <FiZap size={12} className="text-amber-400 fill-amber-400"/>
              <span>PREMIUM SMARTWATCH</span>
            </motion.div>

            {/* Large Luxury Headline */}
            <motion.h1 initial={{ opacity: 0, x: -28 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1, duration: 0.6 }} className="text-[44px] sm:text-[60px] lg:text-[68px] font-black leading-[1.04] tracking-tight mb-6">
            ...
            {/* Supporting Copy */}
            <motion.p initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2, duration: 0.6 }} className="text-slate-300 text-[15px] sm:text-[17px] leading-relaxed mb-10 max-w-md mx-auto lg:mx-0 font-normal tracking-wide">
              Experience a smarter way to stay connected, active, and in control.
            </motion.p>

            {/* CTA Buttons */}
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.6 }} className="flex flex-wrap items-center justify-center lg:justify-start gap-4">
              <Link to="/products" className="inline-flex items-center gap-3 bg-gradient-to-r from-amber-400 via-amber-500 to-yellow-600 hover:from-amber-300 hover:to-yellow-500 text-slate-950 px-8 py-4 rounded-2xl font-black text-sm tracking-wide shadow-[0_10px_30px_rgba(212,175,55,0.35)] hover:shadow-[0_15px_35px_rgba(212,175,55,0.5)] transition-all transform hover:-translate-y-0.5 active:translate-y-0">
                <span>Shop Now</span>
                <FiArrowRight size={17} strokeWidth={2.5}/>
              </Link>

              <Link to="/products" className="inline-flex items-center gap-2.5 bg-white/5 hover:bg-white/10 text-white border border-white/15 px-8 py-4 rounded-2xl font-bold text-sm backdrop-blur-md transition-all transform hover:-translate-y-0.5">
          ...
          <div className="flex-1 flex justify-center lg:justify-end items-center z-20 w-full">
            <div className="relative w-full max-w-[750px]" style={{ perspective: 1200 }}>
              ...
              {goldParticles.map((p, i) => (<motion.div key={i} animate={{ y: [0, -16, 0], opacity: [0.5, 1, 0.5] }} transition={{ duration: 3.2 + i * 0.7, repeat: Infinity, delay: p.delay, ease: "easeInOut" }} className="absolute rounded-full z-30 pointer-events-none bg-gradient-to-r from-amber-300 to-yellow-400 shadow-[0_0_12px_#F59E0B]" style={{
                width: p.size,
                height: p.size,
                top: p.top,
                left: p.left,
              ...
              <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 4.5, repeat: Infinity, ease: "easeInOut" }} style={{
            transformStyle: "preserve-3d",
            transform: `rotateX(${rotateX}deg) rotateY(${rotateY}deg)`,
            transition: "transform 0.15s cubic-bezier(0.2, 0, 0, 1)",
        }} className="relative z-20 group">
                <img src={heroWatchImg} alt="Premium Smartwatch Collection" className="w-full h-auto object-contain pointer-events-none drop-shadow-[0_25px_60px_rgba(0,0,0,0.9)]" style={{
            ...
            maskImage: "linear-gradient(to right, transparent 0%, black 5%, black 100%)",
              ...
              <div className="w-[500px] h-9 bg-black/90 rounded-full blur-2xl mx-auto -mt-6 pointer-events-none transition-transform duration-150" style={{
      ...
      {/* ═══════════════════════════════════════════
        ...
        ═══════════════════════════════════════════ */}
      {categories.length > 0 && (<section className="py-20" style={{ background: "#FAFAFA" }}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
              {categories.map((cat, i) => (<motion.div key={cat.id} whileHover={{ y: -6, scale: 1.02 }} transition={{ type: "spring", stiffness: 400 }}>
                  <Link to={`/products?category=${cat.id}`} className="flex flex-col items-center gap-3 py-6 px-3 bg-white rounded-3xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-[0_8px_30px_rgba(99,102,241,0.12)] transition text-center group">
                    <div className="h-14 w-14 rounded-2xl flex items-center justify-center text-white text-2xl shadow-md" style={{ background: `linear-gradient(135deg, ${categoryColors[i % 6].from}, ${categoryColors[i % 6].to})` }}>
                      <FiPackage size={22}/>
                    </div>
      ...
      {/* ═══════════════════════════════════════════
        ...
        ═══════════════════════════════════════════ */}
      {featuredProducts.length > 0 && (<section className="py-12 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-end justify-between mb-10">
              <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {featuredProducts.map((product, i) => (<motion.div key={product.id} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} whileHover={{ y: -8 }} className="bg-white rounded-3xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.04)] hover:shadow-[0_12px_40px_rgba(99,102,241,0.15)] transition-all duration-250 group relative overflow-hidden flex flex-col">
                  {/* Image */}
                  <Link to={`/product/${product.slug || product.id}`} className="block relative h-52 overflow-hidden" style={{ background: "linear-gradient(135deg, #F0F0FF, #E8ECFF)" }}>
                    {product.images?.[0] ? (<img src={product.images[0]} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-500"/>) : (<div className="w-full h-full flex items-center justify-center">
                        <FiPackage size={48} className="text-indigo-200"/>
                      </div>)}

                    {/* Wishlist button overlay */}
                    <button onClick={(e) => toggleWishlist(product.id, e)} className={`absolute top-3 right-3 h-8 w-8 rounded-full flex items-center justify-center transition shadow-md ${wishlist.has(product.id)
                    ...
                    {/* Category badge */}
                    {product.category?.name && (<span className="absolute top-3 left-3 bg-white/90 backdrop-blur text-indigo-600 text-[9px] font-black uppercase tracking-wider px-2 py-1 rounded-full shadow-sm">
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-between">
                    <div>
                      <Link to={`/product/${product.slug || product.id}`} className="text-sm font-black text-slate-800 hover:text-indigo-600 transition block line-clamp-1 mb-1.5">
                        {product.name}
                      </Link>

                      {/* Stars */}
                      <div className="flex items-center gap-0.5 mb-3">
                        {[...Array(5)].map((_, j) => (<FiStar key={j} size={10} className={j < 4 ? "text-amber-400" : "text-slate-200"} fill={j < 4 ? "currentColor" : "none"}/>))}
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <button onClick={(e) => handleAddToCart(product, e)} className="flex items-center gap-1.5 px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-black rounded-xl shadow shadow-indigo-500/20 transition hover:scale-[1.05] active:scale-[0.95]">
      ...
      <section className="py-10 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative rounded-3xl overflow-hidden px-10 py-14 flex flex-col md:flex-row items-center justify-between gap-8" style={{ background: "linear-gradient(135deg, #4F46E5 0%, #7C3AED 60%, #6D28D9 100%)" }}>
            {/* Deco circles */}
            ...
            <div className="relative z-10 shrink-0">
              <Link to="/products" className="inline-flex items-center gap-2.5 bg-white hover:bg-indigo-50 text-indigo-700 font-black px-8 py-4 rounded-2xl shadow-xl text-sm transition hover:scale-[1.03]">
                <span>Shop Now</span>
