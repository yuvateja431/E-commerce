...
import { FiFilter, FiSearch, FiX, FiCheck, FiShoppingCart, FiHeart, FiEye, FiStar } from "react-icons/fi";
...
export const AccessoriesPage = () => {
    ...
    const initPage = async () => {
      ...
      try {
        ...
        const filteredCats = allCats.filter((cat: any) =>
          accessoryCategoryNames.includes(cat.name)
        );
  ...
  const fetchProducts = async (catsList: any[], categoryFilter: string, sorting: string) => {
    setLoading(true);
    try {
      let categoryIds = "";
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] font-sans antialiased pb-24">
      {/* Hero Banner Section */}
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 mb-8 flex flex-col lg:flex-row gap-6 justify-between items-center">
          ...
          <div className="flex flex-wrap gap-2 w-full lg:w-auto">
            <button
              ...
              className={`px-5 py-2.5 rounded-2xl font-bold text-xs uppercase tracking-wider transition ${selectedCategory === "All"
            ...
            {accessoryCategoryNames.map((catName) => (
              <button
                key={catName}
                onClick={() => handleCategoryChange(catName)}
                className={`px-5 py-2.5 rounded-2xl font-bold text-xs uppercase tracking-wider transition ${selectedCategory === catName
                    ? "bg-indigo-600 text-white shadow-md shadow-indigo-600/10"
                    : "bg-gray-50 text-gray-600 hover:bg-gray-100"
                  }`}
              >
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
            ...
            <div className="relative flex-1 sm:w-80">
              ...
              <input
                ...
                className="w-full pl-11 pr-10 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm transition"
              />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery("")}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 p-1 rounded-full hover:bg-gray-200"
                >
                  <FiX size={14} />
                </button>
              )}
            ...
            <select
              ...
              className="px-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 text-sm text-gray-700 font-bold tracking-wide"
            >
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="h-[380px] bg-white border border-gray-100 rounded-3xl p-4 flex flex-col space-y-4 animate-pulse">
            ...
            {filteredProducts.map((product) => {
              ...
              return (
                <motion.div
                  ...
                  className="group bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden flex flex-col relative"
                >
                  {/* Badges */}
                  <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
                    {hasDiscount && (
                      <span className="bg-red-500 text-white font-extrabold text-[10px] uppercase tracking-wider px-2.5 py-1 rounded-full shadow-sm">
                        Sale -{Math.round(((product.price - product.discountPrice) / product.price) * 100)}%
                      </span>
                    )}
                    {product.averageRating >= 4.8 && (
                      <span className="bg-amber-500 text-white font-extrabold text-[10px] uppercase tracking-wider px-2.5 py-1 rounded-full shadow-sm flex items-center gap-1">
                  ...
                  <button
                    ...
                    className={`absolute top-4 right-4 z-10 p-2.5 rounded-full shadow-sm transition-all duration-300 ${isWishlisted
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    ...
                    <Link
                      ...
                      className="font-extrabold text-gray-900 hover:text-indigo-600 transition text-sm mb-2 line-clamp-2 block"
                    >
                      {product.name}
                    </Link>
                    ...
                    <div className="flex justify-between items-center mt-auto pt-4 border-t border-gray-50">
                      ...
                      <button
                        ...
                        className="bg-indigo-600 text-white p-2.5 rounded-2xl hover:bg-indigo-700 transition-all duration-300 shadow-md shadow-indigo-600/10 hover:shadow-indigo-600/20"
                        title="Add to Cart"
                      >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              ...
              <button
                ...
                className="absolute top-4 right-4 z-10 p-2 bg-gray-100 hover:bg-gray-200 text-gray-600 hover:text-gray-900 rounded-full transition"
              >
                <FiX size={18} />
              </button>
              ...
              <div className="md:w-1/2 p-6 flex flex-col bg-gray-50/50 justify-center">
                <div className="aspect-square w-full rounded-2xl overflow-hidden bg-white border border-gray-100 shadow-sm mb-4 flex items-center justify-center">
                  <img
                    src={quickViewProduct.images?.[activeImageIndex] || "https://placehold.co/400x400/png?text=No+Image"}
                ...
                {quickViewProduct.images && quickViewProduct.images.length > 1 && (
                  <div className="flex gap-2.5 overflow-x-auto py-1">
                    {quickViewProduct.images.map((imgUrl: string, idx: number) => (
                      <button
                        key={idx}
                        onClick={() => setActiveImageIndex(idx)}
                        className={`w-16 h-16 rounded-xl overflow-hidden border-2 transition-all flex-shrink-0 bg-white ${idx === activeImageIndex ? "border-indigo-600 scale-95" : "border-gray-200"
                          }`}
                      >
              ...
              <div className="md:w-1/2 p-8 overflow-y-auto max-h-[80vh] md:max-h-none flex flex-col">
                  ...
                  <div className="mb-6 space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {quickViewProduct.variants.map((variant: any) => (
                        <button
                          key={variant.id}
                          onClick={() => setSelectedVariant(prev => ({ ...prev, [variant.name]: variant.value }))}
                          className={`px-3 py-1.5 rounded-lg border text-xs font-bold transition ${selectedVariant[variant.name] === variant.value
                ...
                <div className="flex gap-4">
                  <button
                    ...
                    className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold text-sm py-4 rounded-2xl transition shadow-lg shadow-indigo-600/10 flex items-center justify-center gap-2 disabled:opacity-50"
                  >
                    <FiShoppingCart /> Add To Shopping Cart
                  </button>
                  <button
                    onClick={(e) => handleToggleWishlist(e, quickViewProduct.id)}
                    className={`p-4 border rounded-2xl transition ${isInWishlist(quickViewProduct.id)
