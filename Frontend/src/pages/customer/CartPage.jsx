...
export const CartPage = () => {
    ...
    const subtotal = items.reduce((acc, item) => acc + ((item?.product?.price ?? 0) * (item?.quantity ?? 0)), 0);
    ...
    return (<div className="max-w-7xl mx-auto px-4 py-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-6">
          {items.map((item) => (<div key={item.id} className="flex gap-6 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 items-center">
              <div className="w-24 h-24 bg-gray-50 rounded-xl overflow-hidden flex-shrink-0">
                <img src={item?.product?.images?.[0] || "https://placehold.co/400x400"} alt={item?.product?.name} className="w-full h-full object-cover"/>
              </div>

              <div className="flex-grow">
                <div className="flex justify-between items-start mb-2">
                  <Link to={`/product/${item?.product?.id}`} className="font-bold text-lg text-gray-900 hover:text-indigo-600 transition">
                    {item?.product?.name}
                  </Link>
                  <button onClick={() => handleRemove(item?.product?.id)} className="text-gray-400 hover:text-red-500 transition">
                    <FiTrash2 size={20}/>
                  </button>
                </div>
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                    <button onClick={() => handleUpdateQuantity(item?.product?.id, item.quantity - 1)} className="px-3 py-1 hover:bg-gray-50 text-gray-600">
                      <FiMinus size={14}/>
                    </button>
          ...
          <div className="flex justify-between pt-6">
            ...
            <button onClick={() => dispatch(clearCart())} className="text-gray-500 font-medium hover:text-red-500 transition">
              Clear Cart
            </button>
          </div>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 sticky top-24">
            ...
            <Link to="/checkout" className="w-full bg-indigo-600 text-white py-4 rounded-xl font-bold hover:bg-indigo-700 transition flex items-center justify-center gap-2">
