...
export const ReturnsRefundsPage: React.FC = () => {
  ...
  useEffect(() => {
    document.title = "Returns & Refunds | E-Commerce Store";
    fetchData();
  }, []);
  ...
  const fetchData = async () => {
    try {
      ...
      setError(err?.response?.data?.message || "Failed to load Returns & Refunds policy. Please try again.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-4">
          ...
          <h1 className="text-3xl sm:text-4xl font-black text-slate-900 tracking-tight">
            {page?.pageTitle || "Returns & Refunds Policy"}
          </h1>
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-16 space-y-3">
            ...
            <p className="text-sm font-medium text-slate-500">Loading Returns & Refunds Policy...</p>
          </div>
        )}
        ...
        {/* Main Content Card */}
        {!loading && !error && page && (
          <div className="bg-white border border-slate-200/80 rounded-2xl p-6 sm:p-10 shadow-2xs space-y-6">
            <div
              className="prose prose-slate max-w-none prose-headings:font-bold prose-headings:text-slate-900 prose-p:text-slate-600 prose-p:leading-relaxed prose-li:text-slate-600"
