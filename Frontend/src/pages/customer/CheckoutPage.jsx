...
import { FiMapPin, FiLock, FiArrowRight, FiShoppingBag, FiTag, FiShield, FiRotateCcw, FiCheck, FiHome, FiBriefcase, FiAlertCircle, } from "react-icons/fi";
...
const UpiLogo = ({ className = "h-5" }) => (<div className={`flex items-center gap-0.5 font-black text-slate-800 tracking-tighter italic text-base ${className}`}>
...
const MastercardLogo = () => (<div className="flex items-center justify-center">
    <div className="w-3.5 h-3.5 rounded-full bg-[#EB001B]"/>
    <div className="w-3.5 h-3.5 rounded-full bg-[#F79E1B] -ml-1.5 opacity-90"/>
  </div>);
const BanknoteIcon = ({ className = "w-7 h-7 text-emerald-500" }) => (<svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
    <rect x="2" y="6" width="20" height="12" rx="2"/>
    <circle cx="12" cy="12" r="3"/>
    <path strokeLinecap="round" d="M6 12h.01M18 12h.01"/>
  </svg>);
const CardOutlineIcon = ({ className = "w-7 h-7 text-indigo-600" }) => (<svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
...
export const CheckoutPage = () => {
    ...
    const { instantOrder } = useSelector((state) => state.checkout || { instantOrder: null });
    ...
    const subtotal = items.reduce((acc, item) => {
        const itemPrice = item.variantId && item.product.variants
            ? (item.product.discountPrice || item.product.price) + (item.product.variants.find((v) => v.id === item.variantId)?.additionalPrice || 0)
    ...
    if (instantOrder && !instantProduct && fetchingProduct) {
        return (<div className="max-w-7xl mx-auto px-4 py-20 text-center animate-pulse">
    ...
    return (<div className="min-h-screen bg-[#F8FAFC] py-10 px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 xl:col-span-8 space-y-6">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="bg-white p-7 sm:p-8 rounded-3xl border border-slate-100 shadow-[0_2px_15px_rgba(0,0,0,0.03)]">
                <div className="flex items-center justify-between gap-3 mb-7">
                  ...
                  <button type="button" onClick={() => setIsLocationModalOpen(true)} className="px-3.5 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 rounded-xl text-xs font-bold transition flex items-center gap-1.5 shrink-0">
                    <FiMapPin size={14}/> Update Location
                  </button>
                </div>

                {/* Auto-applied Delivery Address Notification */}
                {(watch("city") || deliveryLocation?.city) ? (<div className="mb-6 p-4 rounded-2xl bg-emerald-50/80 border border-emerald-200/80 flex items-center justify-between gap-3">
                  ...
                  </div>) : (<div className="mb-6 p-4 rounded-2xl bg-amber-50 border border-amber-200 flex items-center gap-3">
                ...
                {savedAddresses.length > 0 && (<div className="mb-6 space-y-2.5">
                ...
                return (<div key={addr.id} onClick={() => {
                        ...
                        setValue("zipCode", addr.postalCode || "");
                        setValue("country", addr.country || "India");
                    }} className={`p-3.5 rounded-2xl border cursor-pointer transition flex items-start justify-between ${isSelected
                        ...
                        : "border-slate-200 hover:border-slate-300 hover:bg-slate-50"}`}>
                            ...
                            {isSelected && (<div className="h-5 w-5 rounded-full bg-indigo-600 text-white flex items-center justify-center shrink-0">
                                <FiCheck size={12}/>
                              </div>)}
                          </div>);
            })}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <div className="relative">
                      <input {...register("street")} className="w-full px-4 py-3.5 bg-slate-50/50 border border-slate-200 rounded-2xl focus:bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/10 outline-none transition text-sm font-semibold text-slate-800 placeholder-slate-400" placeholder="House / Flat No., Street, Area"/>
                  ...
                  <div>
                    ...
                    <input {...register("city")} className="w-full px-4 py-3.5 bg-slate-50/50 border border-slate-200 rounded-2xl focus:bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/10 outline-none transition text-sm font-semibold text-slate-800 placeholder-slate-400" placeholder="City e.g. Hyderabad"/>
                  ...
                  <div>
                    ...
                    <input {...register("state")} className="w-full px-4 py-3.5 bg-slate-50/50 border border-slate-200 rounded-2xl focus:bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/10 outline-none transition text-sm font-semibold text-slate-800 placeholder-slate-400" placeholder="State e.g. Telangana"/>
                  ...
                  <div>
                    ...
                    <input {...register("zipCode")} className="w-full px-4 py-3.5 bg-slate-50/50 border border-slate-200 rounded-2xl focus:bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/10 outline-none transition text-sm font-semibold text-slate-800 placeholder-slate-400" placeholder="500034"/>
                  ...
                  <div>
                    ...
                    <input {...register("country")} className="w-full px-4 py-3.5 bg-slate-50/50 border border-slate-200 rounded-2xl focus:bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/10 outline-none transition text-sm font-semibold text-slate-800 placeholder-slate-400" placeholder="India"/>
              ...
              <div className="bg-white p-7 sm:p-8 rounded-3xl border border-slate-100 shadow-[0_2px_15px_rgba(0,0,0,0.03)]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  {/* Option 1: CARD */}
                  <label className={`relative flex flex-col justify-between p-5 rounded-2xl border-2 cursor-pointer transition-all duration-200 min-h-[115px] ${selectedPayment === "CARD"
            ...
            : "border-slate-200/80 bg-white hover:border-slate-300"}`}>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition ${selectedPayment === "CARD" ? "border-indigo-600 bg-indigo-600" : "border-slate-300"}`}>
                  ...
                  {/* Option 2: UPI */}
                  <label className={`relative flex flex-col justify-between p-5 rounded-2xl border-2 cursor-pointer transition-all duration-200 min-h-[115px] ${selectedPayment === "UPI"
            ? "border-indigo-600 bg-white shadow-md ring-1 ring-indigo-600/20"
            : "border-slate-200/80 bg-white hover:border-slate-300"}`}>
                    <input type="radio" value="UPI" {...register("paymentMethod")} className="sr-only"/>
                    <div className="flex items-center justify-between">
                      <UpiLogo className="h-6"/>
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition ${selectedPayment === "UPI" ? "border-indigo-600 bg-indigo-600" : "border-slate-300"}`}>
                  ...
                  {/* Option 3: COD */}
                  <label className={`relative flex flex-col justify-between p-5 rounded-2xl border-2 cursor-pointer transition-all duration-200 min-h-[115px] ${selectedPayment === "COD"
            ? "border-indigo-600 bg-white shadow-md ring-1 ring-indigo-600/20"
            : "border-slate-200/80 bg-white hover:border-slate-300"}`}>
                    <input type="radio" value="COD" {...register("paymentMethod")} className="sr-only"/>
                    <div className="flex items-center justify-between">
                      <BanknoteIcon className="w-8 h-8 text-emerald-500"/>
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition ${selectedPayment === "COD" ? "border-indigo-600 bg-indigo-600" : "border-slate-300"}`}>
              ...
              {/* Submit Button */}
              <button type="submit" disabled={loading} className="w-full bg-[#4F46E5] hover:bg-indigo-700 text-white py-4.5 px-6 rounded-2xl font-black text-base transition duration-200 shadow-lg shadow-indigo-200 flex items-center justify-between disabled:opacity-50">
          ...
          <div className="lg:col-span-5 xl:col-span-4 space-y-6 sticky top-24">
            ...
            const itemPrice = item.variantId && item.product.variants
                ? (item.product.discountPrice || item.product.price) + (item.product.variants.find((v) => v.id === item.variantId)?.additionalPrice || 0)
                : (item.product.discountPrice || item.product.price);
            return (<div key={item.id} className="flex items-center gap-3.5 p-2 rounded-2xl border border-slate-100 hover:border-indigo-100 transition">
                      <img src={item.product.images?.[0] || "https://placehold.co/100x100"} alt={item.product.name} className="w-16 h-16 rounded-2xl object-cover bg-slate-50 border border-slate-100 shrink-0"/>
                ...
                <div className="pt-3">
                  {!appliedCoupon ? (<div className="flex gap-2">
                      <div className="relative flex-1">
                        ...
                        <input type="text" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} placeholder="DISCOUNT CODE" className="w-full pl-10 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-indigo-600 outline-none text-xs font-bold tracking-wider uppercase"/>
                      </div>
                      <button type="button" onClick={handleApplyCoupon} disabled={validatingCoupon || !couponCode} className="px-4 py-2.5 bg-[#4F46E5] hover:bg-indigo-700 text-white rounded-xl text-xs font-extrabold transition shadow-xs disabled:opacity-50">
                        {validatingCoupon ? "..." : "Apply"}
                      </button>
                    </div>) : (<div className="flex items-center justify-between bg-emerald-50 p-3 rounded-xl border border-emerald-100">
                      <div>
                      ...
                      <button type="button" onClick={handleRemoveCoupon} className="text-xs font-extrabold text-rose-500 hover:text-rose-700">
                        Remove
                      </button>
                    </div>)}
                </div>

                {discountAmount > 0 && (<div className="flex justify-between font-medium text-emerald-600 text-sm">
                    <span>Discount</span>
