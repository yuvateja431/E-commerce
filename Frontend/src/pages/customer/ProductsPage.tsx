...
export const ProductsPage = () => {
  ...
  const toggleArrayParam = (paramName: string, currentValues: string[], value: string) => {
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
        ...
        <div className="relative w-full md:w-1/2 lg:w-1/3 z-50" ref={searchRef}>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (e.target.value === "") updateParams({ search: null });
            }}
            onKeyDown={handleSearchSubmit}
            onFocus={() => { if (suggestions.length) setShowSuggestions(true); }}
            className="w-full pl-10 pr-10 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm"
          />
          {searchInput && (
            <button 
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2.5 border border-gray-200 rounded-xl hover:bg-gray-50 md:hidden font-medium text-gray-700 bg-white"
          >
            <FiFilter /> Filters
          </button>
          ...
          <select 
            ...
            className="flex-1 md:flex-none px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white text-gray-700 font-medium shadow-sm"
          >
                ...
                {categories.map((cat: any) => (
                  <label key={cat.id} className="flex items-center gap-3 cursor-pointer group">
                    ...
                    <input 
                      ...
                      onChange={() => toggleArrayParam("category", selectedCategories, cat.id)}
                    />
                ...
                {predefinedBrands.map(b => (
                  <label key={b} className="flex items-center gap-3 cursor-pointer group">
                    ...
                    <input 
                      ...
                      onChange={() => toggleArrayParam("brand", selectedBrands, b)}
                    />
                ...
                {[4, 3, 2, 1].map(r => (
                  <label key={r} className="flex items-center gap-2 cursor-pointer">
                    <input 
                      ...
                      onChange={() => updateParams({ rating: r.toString() })}
        ...
        <div className="flex-1 w-full">
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {[...Array(8)].map((_, i) => (
                <div key={i} className="h-[380px] bg-white border border-gray-100 rounded-2xl animate-pulse p-4 flex flex-col">
            ...
            <div className="text-center py-24 bg-white rounded-3xl border border-gray-100 border-dashed">
              ...
              <button 
                ...
                className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition shadow-sm"
              >
                Clear All Filters
              </button>
            </div>
              ...
              {pagination.totalPages > 1 && (
                <div className="mt-16 flex justify-center gap-2">
                  <button
                    ...
                    className="px-4 py-2 rounded-xl font-medium border border-gray-200 text-gray-600 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 bg-white"
                  >
                    Previous
                  </button>
                  ...
                  {Array.from({ length: pagination.totalPages }, (_, i) => i + 1).map(p => (
                    <button
                      ...
                      className={`h-10 w-10 rounded-xl flex items-center justify-center font-bold transition shadow-sm ${
                        pagination.page === p ? "bg-indigo-600 text-white border-transparent" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"
                      }`}
                    >
                      {p}
                    </button>
