...
import { FiSearch, FiMoreVertical, FiCalendar, FiChevronLeft, FiChevronRight, FiCheck, FiTrash2, FiFilter, FiDownload, FiAlertTriangle, FiX } from "react-icons/fi";
...
export const AdminUsersPage = () => {
  ...
  const [deleteConfirmUser, setDeleteConfirmUser] = useState<{ id: string; name: string; email: string } | null>(null);
  ...
  const filteredUsers = users
    .filter((u: any) => !excludedEmails.includes(u.email?.toLowerCase()))
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200/80 rounded-2xl text-xs font-semibold text-slate-700 hover:bg-slate-50 shadow-2xs transition cursor-pointer"
          >
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="relative inline-block text-left" ref={activeDropdownId === user.id ? dropdownRef : null}>
                      <button
                        ...
                        className="p-2 border border-slate-200/80 rounded-xl hover:bg-slate-100 text-slate-500 transition cursor-pointer"
                        title="Actions"
                      >
                      ...
                      {activeDropdownId === user.id && (
                        <div className="absolute right-0 mt-2 w-44 bg-white border border-slate-100 rounded-2xl shadow-xl z-30 py-1.5 animate-in fade-in duration-150">
                          ...
                          <button
                            onClick={() => { handleUpdateRole(user.id, "USER"); setActiveDropdownId(null); }}
                            className={`w-full text-left px-3.5 py-2 text-xs font-semibold hover:bg-blue-50 flex items-center justify-between cursor-pointer ${
                              user.role === 'USER' ? 'text-blue-600 font-bold' : 'text-slate-700'
                            }`}
                          >
                            <span>USER</span>
                          ...
                          <button
                            onClick={() => { handleUpdateRole(user.id, "ADMIN"); setActiveDropdownId(null); }}
                            className={`w-full text-left px-3.5 py-2 text-xs font-semibold hover:bg-purple-50 flex items-center justify-between cursor-pointer ${
                              user.role === 'ADMIN' ? 'text-purple-700 font-bold' : 'text-slate-700'
                            }`}
                          >
                            <span>ADMIN</span>
                          ...
                          <div className="border-t border-slate-100 mt-1 pt-1">
                            <button
                              onClick={() => { promptDeleteUser(user); setActiveDropdownId(null); }}
                              className="w-full text-left px-3.5 py-2 text-xs font-bold text-red-600 hover:bg-red-50 flex items-center gap-2 cursor-pointer transition"
                            >
                              <FiTrash2 size={13} />
