...
import { FiArrowUp, FiArrowDown, FiMoreVertical, FiSearch, FiChevronLeft, FiChevronRight, FiEdit2, FiRefreshCw } from "react-icons/fi";
    ...
    return (<div className="space-y-6 pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input type="text" placeholder="Search product name or ID..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="w-full pl-9 pr-3.5 py-2 text-xs bg-white border border-slate-200/90 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition shadow-2xs placeholder-slate-400 font-normal"/>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[700px]">
            ...
            <tbody className="divide-y divide-slate-100/80">
              {loading ? ([1, 2, 3, 4, 5].map(n => (<tr key={n}>
                    <td colSpan={4} className="px-6 py-4">
                      <div className="h-10 bg-slate-100 animate-pulse rounded-xl"/>
                    </td>
                  </tr>))) : paginatedProducts.length === 0 ? (<tr>
                ...
                </tr>) : (paginatedProducts.map((product) => {
            const stock = product.inventory?.stock ?? 0;
            const threshold = product.inventory?.lowStockThreshold ?? 10;
            const isLow = stock <= threshold && stock > 0;
            const isOutOfStock = stock === 0;
            return (<tr key={product.id} className="hover:bg-slate-50/60 transition-colors duration-150 group">
                      {/* PRODUCT Column */}
                      <td className="px-6 py-4.5">
                        <div className="flex items-center gap-3.5">
                          <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-200/60 overflow-hidden flex items-center justify-center shrink-0 shadow-2xs">
                            <img src={product.images?.[0] || product.image || "https://placehold.co/48x48"} alt={product.name} className="w-full h-full object-contain p-1"/>
                          </div>
                    ...
                    display: 'block'
                      ...
                      {/* STATUS Column */}
                      <td className="px-6 py-4.5">
                        {isOutOfStock ? (<span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-extrabold uppercase tracking-wider bg-rose-50 text-rose-600 border border-rose-100/80">
                            <span className="w-1.5 h-1.5 rounded-full bg-rose-500 shrink-0"/>
                            OUT OF STOCK
                          </span>) : isLow ? (<span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-extrabold uppercase tracking-wider bg-amber-50 text-amber-600 border border-amber-100/80">
                            <span className="w-1.5 h-1.5 rounded-full bg-amber-500 shrink-0"/>
                            LOW STOCK
                          </span>) : (<span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-extrabold uppercase tracking-wider bg-emerald-50 text-emerald-600 border border-emerald-100/80">
                      ...
                      <td className="px-6 py-4.5 text-right">
                        <div className="flex items-center justify-end gap-2.5">
                          {/* Restock (+10) Button */}
                          <button onClick={() => handleUpdateStock(product.id, 10)} className="bg-white hover:bg-indigo-50/70 border border-indigo-200/90 text-indigo-600 font-bold text-xs px-3.5 py-1.5 rounded-xl shadow-2xs hover:border-indigo-300 transition duration-150 flex items-center gap-1.5 cursor-pointer">
                            <FiArrowUp size={14} className="text-indigo-600"/>
                            <span>Restock (+10)</span>
                          </button>

                          {/* Correct (-1) Button */}
                          <button onClick={() => handleUpdateStock(product.id, -1)} className="bg-white hover:bg-slate-50 border border-slate-200/90 text-slate-600 font-bold text-xs px-3.5 py-1.5 rounded-xl shadow-2xs hover:border-slate-300 transition duration-150 flex items-center gap-1.5 cursor-pointer">
                    ...
                    setActiveMenuId(activeMenuId === product.id ? null : product.id);
                }} className="p-1.5 rounded-xl text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition cursor-pointer" title="More actions">
                              <FiMoreVertical size={16}/>
                            </button>

                            {/* Dropdown Menu Popup */}
                            {activeMenuId === product.id && (<div onClick={(e) => e.stopPropagation()} className="absolute right-0 top-9 w-48 bg-white rounded-2xl shadow-xl border border-slate-100 py-1.5 z-30 animate-in fade-in zoom-in-95 duration-150 text-left">
                                <button onClick={() => {
                        ...
                        handleUpdateStock(product.id, 50);
                                ...
                                <Link to={`/admin/products`} className="w-full px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-indigo-50 hover:text-indigo-600 flex items-center gap-2 transition cursor-pointer">
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 bg-white">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              <button disabled={currentPage === 1} onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} className="p-1.5 rounded-xl border border-slate-200 text-slate-400 hover:bg-slate-50 disabled:opacity-40 transition cursor-pointer">
                <FiChevronLeft size={15}/>
              </button>

              {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (<button key={page} onClick={() => setCurrentPage(page)} className={`w-8 h-8 rounded-xl text-xs font-extrabold flex items-center justify-center transition cursor-pointer ${page === currentPage
                ? "bg-indigo-600 text-white shadow-xs"
                : "border border-slate-200 text-slate-600 hover:bg-slate-50"}`}>
                  {page}
                </button>))}

              <button disabled={currentPage >= totalPages} onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} className="p-1.5 rounded-xl border border-slate-200 text-slate-400 hover:bg-slate-50 disabled:opacity-40 transition cursor-pointer">
