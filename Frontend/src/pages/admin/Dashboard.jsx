...
import { FiTrendingUp, FiShoppingBag, FiUsers, FiDollarSign, FiChevronDown, FiArrowUpRight, FiArrowDownRight, FiActivity, FiTag, FiShoppingCart, FiChevronRight } from "react-icons/fi";
...
export const AdminDashboard = () => {
    ...
    if (!data || !data.stats)
        return (<div className="text-center p-8 bg-gray-50 border border-gray-100 rounded-2xl text-gray-500">
      No data available
    </div>);
    ...
    const customSalesData = data.salesData?.length > 0
        ? data.salesData.map((d) => ({ name: d.month, revenue: d.revenue }))
        : [
    ...
    return (<div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (<div key={i} className="bg-white p-6 rounded-[24px] border border-gray-100/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex flex-col justify-between hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition duration-200">
              ...
              <div className="w-16 h-8 overflow-hidden shrink-0">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={stat.sparkData} margin={{ top: 2, bottom: 2, left: 2, right: 2 }}>
                    <Area type="monotone" dataKey="val" stroke={stat.color} strokeWidth={1.5} fillOpacity={0.06} fill={stat.color}/>
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-[24px] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] p-6 flex flex-col justify-between h-full">
          <div>
            ...
            <div className="space-y-4">
              {mockTopProducts.map((p, idx) => (<div key={idx} className="flex items-center justify-between gap-4 group">
                  ...
                  <div className="flex flex-col items-end shrink-0">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden hidden sm:block">
                        <div className="h-full bg-indigo-600 rounded-full" style={{ width: `${p.percentage}%` }}/>
                      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Link to="/admin/orders" className="bg-white p-5 rounded-[22px] border border-gray-100/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex items-center justify-between hover:shadow-[0_8px_30px_rgb(0,0,0,0.05)] transition duration-200">
          <div className="flex items-center gap-4">
        ...
        <Link to="/admin/inventory" className="bg-white p-5 rounded-[22px] border border-gray-100/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex items-center justify-between hover:shadow-[0_8px_30px_rgb(0,0,0,0.05)] transition duration-200">
          <div className="flex items-center gap-4">
        ...
        <Link to="/admin/coupons" className="bg-white p-5 rounded-[22px] border border-gray-100/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex items-center justify-between hover:shadow-[0_8px_30px_rgb(0,0,0,0.05)] transition duration-200">
          <div className="flex items-center gap-4">
        ...
        <Link to="/admin/users" className="bg-white p-5 rounded-[22px] border border-gray-100/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex items-center justify-between hover:shadow-[0_8px_30px_rgb(0,0,0,0.05)] transition duration-200">
          <div className="flex items-center gap-4">
