...
export const CouponDetailsPage = () => {
    ...
    if (error || !coupon)
        return <div className="p-8 text-center text-red-500">{error || "Coupon not found"}</div>;
    const totalDiscountGiven = coupon.orders?.reduce((sum, order) => {
    ...
    return (<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <Link to={`/admin/coupons/${coupon.id}/edit`} className="bg-indigo-50 text-indigo-600 px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-indigo-100 transition font-medium">
          <FiEdit /> Edit
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            {coupon.orders?.length > 0 ? (<div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {coupon.orders.map((order) => (<tr key={order.id} className="hover:bg-gray-50 transition-colors">
              ...
              </div>) : (<p className="text-gray-500 text-center py-8">This coupon hasn't been used yet.</p>)}
          </div>
        </div>
