...
import { getContactMessages, updateContactMessageStatus, deleteContactMessage, } from "../../services/cmsService";
import { FiSearch, FiEye, FiTrash2, FiMessageSquare, FiLoader, FiX, FiCheck, FiClock, FiMail, } from "react-icons/fi";
...
export const AdminContactMessagesPage = () => {
    ...
    const filteredMessages = messages.filter((m) => m.fullName.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-2xs flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="relative w-full sm:w-80">
          ...
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search by name, email, subject..." className="w-full pl-10 pr-4 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition text-slate-800"/>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-2xs overflow-hidden">
        {loading ? (<div className="p-12 text-center text-slate-400 space-y-3">
          ...
          </div>) : filteredMessages.length === 0 ? (<div className="p-12 text-center text-slate-400 space-y-2">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-700">
                {filteredMessages.map((msg) => (<tr key={msg.id} className="hover:bg-slate-50/60 transition">
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => handleViewMessage(msg)} className="p-1.5 hover:bg-blue-50 rounded-lg text-blue-600 transition" title="View Details">
                          <FiEye size={16}/>
                        </button>
                        <select value={msg.status} onChange={(e) => handleUpdateStatus(msg.id, e.target.value)} className="text-[11px] bg-slate-50 border border-slate-200 rounded-lg px-2 py-1 font-bold text-slate-700">
                          <option value="New">New</option>
                          <option value="Read">Read</option>
                          <option value="Replied">Replied</option>
                          <option value="Closed">Closed</option>
                        </select>
                        <button onClick={() => setDeleteMessageId(msg.id)} className="p-1.5 hover:bg-rose-50 rounded-lg text-rose-500 transition" title="Delete Message">
                          <FiTrash2 size={16}/>
                        </button>
                      </div>
                    </td>
      ...
      {/* View Details Modal */}
      {selectedMessage && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              ...
              <button onClick={() => setSelectedMessage(null)} className="p-1 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition">
                <FiX size={18}/>
              </button>
            </div>
            ...
            <div className="p-6 space-y-5 text-xs">
              ...
              <div className="flex items-center justify-between pt-3 border-t border-slate-100 text-[11px] text-slate-500">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => handleUpdateStatus(selectedMessage.id, "Replied")} className="px-3 py-1.5 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 font-bold rounded-lg transition flex items-center gap-1">
                    <FiCheck size={12}/>
                    <span>Mark Replied</span>
                  </button>
                  <button onClick={() => handleUpdateStatus(selectedMessage.id, "Closed")} className="px-3 py-1.5 bg-amber-50 text-amber-700 hover:bg-amber-100 font-bold rounded-lg transition">
                    Close Thread
                  </button>
                </div>
              </div>
      ...
      {deleteMessageId && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-2xl w-full max-w-sm overflow-hidden p-6 text-center space-y-4">
            ...
            <div className="flex items-center justify-center gap-3 pt-2">
              <button onClick={() => setDeleteMessageId(null)} className="px-4 py-2 text-xs font-bold text-slate-600 hover:bg-slate-100 rounded-xl transition">
                Cancel
              </button>
