...
const parseNumberOrNull = (val: unknown) => {
...
const baseSchema = z.object({
  ...
  discountValue: z.preprocess(
    ...
    z.number({ message: "Discount value is required" }).min(0.01, "Value must be greater than 0")
  ),
...
const schema = baseSchema
  .refine(data => {
    if (!data.validFrom || !data.expiryDate) return true;
...
export const CouponFormPage = () => {
  ...
  if (loading) return <div className="p-8 text-center text-slate-500 font-medium">Loading coupon details...</div>;
      ...
      <form onSubmit={handleSubmit(onSubmit, onInvalid)} className="space-y-6">
        <div className="bg-white p-6 rounded-2xl shadow-xs border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                {...register("name")}
                className={`w-full px-4 py-2 border rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none ${errors.name ? 'border-red-500' : 'border-gray-200'}`}
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  {...register("code")}
                  className={`flex-1 px-4 py-2 border rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none uppercase ${errors.code ? 'border-red-500' : 'border-gray-200'}`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none bg-white cursor-pointer"
              >
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2 border rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none ${errors.discountValue ? 'border-red-500' : 'border-gray-200'}`}
                placeholder="e.g. 10"
              />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                type="datetime-local"
                {...register("validFrom")}
                className={`w-full px-4 py-2 border rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none ${errors.validFrom ? 'border-red-500' : 'border-gray-200'}`}
              />
            ...
            <div>
              ...
              <input
                type="datetime-local"
                {...register("expiryDate")}
                className={`w-full px-4 py-2 border rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none ${errors.expiryDate ? 'border-red-500' : 'border-gray-200'}`}
              />
        ...
        <div className="bg-white p-6 rounded-2xl shadow-xs border border-gray-100">
          ...
          <div className="mt-6 pt-6 border-t border-gray-100">
            ...
            <select
              {...register("status")}
              className="w-full md:w-1/2 px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none bg-white cursor-pointer"
            >
        ...
        <div className="flex justify-end gap-4">
          <Link
            to="/admin/coupons"
            className="px-6 py-3 border border-gray-200 text-gray-700 font-medium rounded-xl hover:bg-gray-50 transition cursor-pointer"
          >
            Cancel
          </Link>
          <button
            type="submit"
            disabled={saving}
            className="px-6 py-3 bg-indigo-600 text-white font-medium rounded-xl hover:bg-indigo-700 transition flex items-center gap-2 disabled:opacity-50 cursor-pointer shadow-sm"
          >
