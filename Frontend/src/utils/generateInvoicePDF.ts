...
export const generateInvoicePDF = (order: any, user?: any) => {
  ...
  const fmtCurr = (num: number) => `Rs. ${num.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
  ...
  const drawEnvelopeIcon = (eX: number, eY: number, color: number[]) => {
    setDraw(color);
    doc.setLineWidth(0.35);
