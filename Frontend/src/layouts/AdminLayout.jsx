...
import { FiHome, FiBox, FiLayers, FiShoppingCart, FiUsers, FiTag, FiPackage, FiBarChart2, FiSettings, FiLogOut, FiChevronDown, FiUser, FiBell, FiSearch, FiMenu, FiAlertTriangle, FiMoon, FiSun, FiHelpCircle, FiTruck, FiRefreshCw, FiMail, FiMessageSquare } from "react-icons/fi";
...
export const AdminLayout = () => {
    ...
    return (<div className={`flex h-screen overflow-hidden ${isDarkMode ? "dark bg-[#131b2e] text-slate-100" : "bg-[#F4F6FC] text-slate-800"}`}>

      {/* ════════════════════════════
            SIDEBAR
        ════════════════════════════ */}
      <aside className="relative flex flex-col shrink-0 transition-all duration-300 z-20 bg-white dark:bg-[#1e293b] border-r border-slate-200/80 dark:border-slate-800 shadow-xs" style={{
        ...
        <nav className="flex-1 px-3.5 py-4 space-y-4 overflow-y-auto custom-scrollbar">
          {menuGroups.map((group, groupIdx) => (<div key={groupIdx} className="space-y-1">
              {group.title && !sidebarCollapsed && (<p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider px-3 pt-2 pb-1 text-[#94A3B8]">
                  {group.title}
                </p>)}
              {group.items.map((item) => {
                const isActive = location.pathname === item.path ||
                    (item.path !== "/admin/dashboard" && location.pathname.startsWith(item.path + "/"));
                return (<Link key={item.path} to={item.path} title={sidebarCollapsed ? item.name : undefined} className={`flex items-center justify-between px-3.5 py-2.5 rounded-xl transition-all duration-200 group relative ${isActive
                        ? "bg-[#F0F5FF] text-blue-600 font-bold"
                        : "text-slate-700 hover:bg-slate-50 font-medium"}`}>
                    <div className="flex items-center gap-3">
                      <span className={`text-[17px] shrink-0 transition ${isActive ? "text-blue-600" : "text-slate-500 group-hover:text-blue-600"}`}>
                        {item.icon}
                      </span>
                      {!sidebarCollapsed && (<span className="text-[13.5px] tracking-tight">{item.name}</span>)}
                    </div>

                    {!sidebarCollapsed && item.badge && (<div className="flex items-center gap-1.5">
                        <span className="bg-blue-600 text-white text-[10px] font-black px-2 py-0.5 rounded-full shadow-2xs">
                          {item.badge}
                        </span>
                      </div>)}

                    {sidebarCollapsed && item.badge && (<span className="absolute top-1 right-1 w-2 h-2 bg-blue-600 rounded-full"/>)}
                  </Link>);
            })}
            </div>))}
        </nav>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white dark:bg-[#1e293b] border-b border-slate-100 dark:border-slate-800 shadow-[0_2px_10px_rgba(0,0,0,0.03)] h-[70px] flex items-center px-6 gap-6 shrink-0">
          ...
          {/* Hamburger */}
          <button onClick={() => setSidebarCollapsed(!sidebarCollapsed)} className="p-2 rounded-xl hover:bg-slate-100 text-slate-500 transition">
            <FiMenu size={20}/>
          </button>
          ...
          <div className="flex-1 max-w-sm relative hidden sm:block" ref={searchRef}>
            ...
            setSearchOpen(true);
        }} onFocus={() => setSearchOpen(true)} className="w-full pl-11 pr-5 py-2.5 text-sm text-slate-700 bg-white border border-slate-200/80 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300 transition duration-200 placeholder-slate-400 font-normal shadow-2xs"/>

            {/* Live Search Results Dropdown */}
            {searchOpen && searchQuery.trim().length > 0 && (<div className="absolute left-0 right-0 mt-2 bg-white border border-slate-100 rounded-2xl shadow-xl z-50 overflow-hidden py-2">
                <div className="px-3 py-1.5 text-[10px] font-extrabold text-slate-400 uppercase tracking-wider border-b border-slate-50 mb-1">
                  Sidebar Sections
                </div>

                {matchingMenuItems.length > 0 ? (matchingMenuItems.map((item) => (<button key={item.path} onClick={() => {
                    navigate(item.path);
                    setSearchQuery("");
                    setSearchOpen(false);
                    ...
                    </button>))) : (<div className="px-4 py-3 text-xs text-slate-400 text-center">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <button type="button" onClick={toggleTheme} className="p-2.5 rounded-xl transition cursor-pointer text-slate-500 hover:text-slate-800 dark:text-slate-300 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-[#24304f]" title={isDarkMode ? "Switch to Light Mode" : "Switch to Dark Mode"}>
            ...
            setDropdownOpen(false);
        }} className={`relative p-2.5 rounded-xl transition ${notificationsOpen ? "bg-indigo-50 text-indigo-600" : "hover:bg-slate-100 text-slate-500"}`} title="Notifications">
              <FiBell size={20}/>
              {unreadCount > 0 && (<span className="absolute top-1.5 right-1.5 h-2 w-2 bg-[#6366F1] rounded-full shadow-sm ring-2 ring-white"/>)}
            </button>

            {/* Notifications Dropdown Popup */}
            {notificationsOpen && (<div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white border border-slate-100 rounded-2xl shadow-xl z-50 overflow-hidden">
                <div className="px-4 py-3.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                  <div className="flex items-center gap-2">
                    ...
                    {unreadCount > 0 && (<span className="bg-indigo-100 text-indigo-700 text-[10px] font-extrabold px-2 py-0.5 rounded-full">
                        {unreadCount} new
                      </span>)}
                  </div>
                ...
                <div className="max-h-[320px] overflow-y-auto divide-y divide-slate-50">
                  {notifications.length === 0 ? (<div className="p-6 text-center text-slate-400 text-xs">
                      No notifications right now
                    </div>) : (notifications.map((item) => (<div key={item.id} onClick={() => {
                    if (!item.read) {
                    ...
                    if (item.link)
                        navigate(item.link);
                }} className={`p-3.5 flex items-start gap-3 hover:bg-slate-50 transition cursor-pointer ${!item.read ? "bg-indigo-50/40" : ""}`}>
                        <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 mt-0.5 ${item.type === "order"
                    ...
                    : item.type === "inventory"
                        ...
                        {!item.read && (<span className="w-2 h-2 rounded-full bg-indigo-600 shrink-0 mt-1.5"/>)}
                      </div>)))}
                </div>
          ...
          <div className="relative" ref={dropdownRef}>
            <button onClick={() => setDropdownOpen(prev => !prev)} className="flex items-center gap-3 pl-3 pr-2 py-1.5 rounded-xl hover:bg-slate-50 transition">
              {/* Avatar */}
              ...
              <FiChevronDown className={`text-slate-400 transition-transform duration-200 ${dropdownOpen ? "rotate-180" : ""}`} size={15}/>
            </button>

            {/* Dropdown Menu */}
            {dropdownOpen && (<div className="absolute right-0 mt-2 w-56 bg-white border border-slate-100 rounded-2xl shadow-xl z-50 overflow-hidden">
                ...
                <div className="p-2 space-y-1">
                  <button onClick={() => setDropdownOpen(false)} className="flex items-center gap-3 w-full px-3 py-2.5 text-[13px] font-semibold text-slate-600 rounded-xl hover:bg-indigo-50 hover:text-indigo-600 transition">
                    <FiUser size={15}/>
                    <span>My Profile</span>
                  </button>
                  <button onClick={() => { setDropdownOpen(false); navigate("/admin/settings"); }} className="flex items-center gap-3 w-full px-3 py-2.5 text-[13px] font-semibold text-slate-600 rounded-xl hover:bg-indigo-50 hover:text-indigo-600 transition">
                    <FiSettings size={15}/>
                    <span>Settings</span>
                  </button>
                  <div className="border-t border-slate-100 my-1"/>
                  <button onClick={handleLogout} className="flex items-center gap-3 w-full px-3 py-2.5 text-[13px] font-semibold text-rose-500 rounded-xl hover:bg-rose-50 transition">
                    <FiLogOut size={15}/>
                    <span>Logout</span>
                  </button>
                </div>
