...
export const ProductCard = ({ product }: { product: any }) => {
  ...
  const isInWishlist = wishlist?.items?.some((item: any) => item.productId === product.id);
          ...
          <span className="text-xl font-bold text-gray-900">₹{product.price}</span>
            <div className="flex gap-2">
              <button 
                ...
                className="bg-gray-900 text-white p-2 rounded-lg hover:bg-indigo-600 transition"
                title="Add to Cart"
              >
