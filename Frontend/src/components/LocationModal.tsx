...
export const LocationModal: React.FC<LocationModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleApplyLocation = (e?: React.FormEvent) => {
    ...
    if (/^\d{6}$/.test(query)) {
      ...
      const foundCity = POPULAR_CITIES.find(
        (c) => c.city.toLowerCase() === query.toLowerCase()
      );
      if (foundCity) {
  ...
  const handleSelectCity = (cityObj: typeof POPULAR_CITIES[0]) => {
    dispatch(
      setLocation({
        city: cityObj.city,
        ...
        >
          ...
          <div className="p-6 space-y-6 max-h-[78vh] overflow-y-auto">
            ...
            <form onSubmit={handleApplyLocation} className="space-y-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="e.g. 500034 or Hyderabad"
                  className="flex-1 px-4 py-3 text-sm font-semibold text-slate-800 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition placeholder:text-slate-400"
                />
                <button
                  type="submit"
                  className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm rounded-2xl transition shadow-md shadow-indigo-600/20 active:scale-95 shrink-0"
                >
                  Apply
                </button>
              </div>
            ...
            <button
              ...
              disabled={detecting}
              className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-indigo-50 hover:bg-indigo-100/80 text-indigo-700 font-bold text-xs rounded-2xl transition border border-indigo-100 disabled:opacity-50"
            >
                ...
                {loadingAddresses ? (
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                    {savedAddresses.map((addr) => {
                      const isSelected =
                        currentLocation.addressId === addr.id ||
                  ...
                  <p className="text-xs text-slate-400 italic">No saved addresses found in account.</p>
                )}
              </div>
            ) : (
            ...
            <div className="space-y-2.5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {POPULAR_CITIES.map((c) => {
                  const isSelected =
                    currentLocation.city.toLowerCase() === c.city.toLowerCase() &&
